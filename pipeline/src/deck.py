...
def build() -> str:
    ...
    xs = 0.55
    _stat(s, xs, 1.55, 2.85, "Break-even Y3", "company EBITDA positive in year 3 — second year of reimbursed sales")
    ...
    _stat(s, xs, 5.30, 2.85, f"€{nl['system_saving_per_100']:,} saved", "per 100 patients triaged, at €650/colonoscopy — the payer wants this")
    ...
    _bullets(tf, [
        ...
        ("Its 300k tests/yr claim is impossible.", "Germany's entire organised addressable volume is 174,240/yr — "
         "the company-reported figure is 1.7× the whole market. Pipeline check CHK-02 rejects it; the independent "
    ...
    _para(tf, "Right market later, unaffordable market now: file the German reimbursement application early "
              "(the clock is procedural), enter when new capital or NL cash flow can fund the two-year wait.",
    ...
    _bullets(tf, [
        ("Volume without the barriers.", "71% participation × 9.1% positivity → 159,910 FIT-positives/yr from a "
         "country of 4.95M eligible — 92% of Germany's volume."),
    ...
    _bullets(tf, [
        ("Ramp is the sensitive input.", "At half the benchmark ramp the trough falls to ≈€0.2M — survival, "
         "with nothing spare."),
        ("Delay is the killer when combined.", "Half-ramp AND a 24-month delay → trough −€2.1M: insolvent. "
         "Either alone is survivable; both together are not."),
        ("Price is the cushion.", "Every €10 of price ≈ €0.2–0.3M of annual EBITDA at scale — procurement "
         "discount headroom exists."),
        ("Kill condition, stated up front.", "If reimbursement hasn't landed by month 18, cut in-market spend "
         "and bridge — do not ride the base plan into the trough."),
    ], size=12.5)
    ...
    _footer(s, "Break-even moves Y3 → Y4 at ×75% ramp; beyond Y5 only in the red scenarios.", W, H)
